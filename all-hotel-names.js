...
function renderHotelSelector() {
    ...
    input.addEventListener('input', () => {
        ...
        filtered = hotelNames.filter(h => h.toLowerCase().includes(val));
        renderList();
        showDropdown();
    });
    input.addEventListener('focus', () => {
        filtered = hotelNames.filter(h => h.toLowerCase().includes(input.value.trim().toLowerCase()));
        renderList();
        showDropdown();
    });
